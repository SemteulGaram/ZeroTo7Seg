...
float* ffi_ocr_preprocess(uchar* buf, int* size) {
  ...
  if (rects1.size() >= 2) {
    ...
    copyMakeBorder(img2, img2, 20, 20, 20, 20, BORDER_CONSTANT, Scalar(255, 255, 255));
    int padding = img2.size().width - img3.size().width;
    if (padding > 1) {
      copyMakeBorder(img3, img3, 20, 20, padding / 2, padding - (padding / 2), BORDER_CONSTANT, Scalar(255, 255, 255));
      Mat result;
      ...
      img.release();
      img1.release();
      float* jres = (float*)malloc(sizeof(float) * 8);
      memcpy(jres, output.data(), sizeof(float) * 8);
      return jres;
    }
    else {
      v.clear();
